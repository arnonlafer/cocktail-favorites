...
export function InstructionList({ steps }: Props) {
  return (
    <ol className="divide-y divide-white/6 rounded-2xl border border-white/8 bg-bar-900/60">
      {steps.map((step, i) => (
        <li key={i} className="flex gap-3 px-4 py-3">
          ...
          <span className="flex-1 text-sm leading-relaxed text-white/90">{step}</span>
        </li>
      ))}
    </ol>
  )
